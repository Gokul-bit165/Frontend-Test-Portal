...
router.post('/google', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('DB lookup error, fallback to JSON file:', dbLookupError.message);
    ...
    if (!user) {
      ...
      try {
        ...
        console.warn('DB create error, persisting to JSON file:', dbCreateError.message);
        const users = loadJSON(usersPath);
        users.push({
          ...newUser,
          created_at: newUser.created_at instanceof Date ? newUser.created_at.toISOString() : newUser.created_at,
          last_login: newUser.last_login instanceof Date ? newUser.last_login.toISOString() : newUser.last_login,
        });
      ...
      try {
        ...
        console.log('DB update failed, updating JSON fallback:', dbUpdateError.message);
        const users = loadJSON(usersPath);
